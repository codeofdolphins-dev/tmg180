  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <textarea
        ...
        className="h-32 resize-none bg-white border border-slate-300 rounded-xl p-4 text-sm text-[#0b1c30] placeholder:text-[#6b7280] outline-none focus:border-[#7800ce] transition-colors"
...
function SectionForm({ def }) {
  ...
  const { status, position, error, isLast, form } = section;
      ...
      <div className="grid grid-cols-[1fr_301px] gap-6 items-start">
        <div className="flex flex-col gap-8">
          ...
          {def.groups.map((group) => (
            <GroupCard key={group.title} group={group} form={form} />
          ))}
